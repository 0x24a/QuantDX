...
def get_crypto_metrics(symbol):
    ...
    try:
        ...
        candles_result = marketAPI.get_candlesticks(instId=symbol, bar="1D", limit="30")
        ...
        rsi_14 = (
            calculate_rsi(close_prices, period=14) if len(close_prices) >= 15 else None
        )
        ...
        return {
            ...
            "price_change_24h_percentage": round(
                price_change_24h / current_price * 100, 5
            ),
            "price_change_7d": round(price_change_7d, 5) if price_change_7d else None,
            "price_change_7d_percentage": (
                round(price_change_7d / current_price * 100, 5)
                if price_change_7d
                else None
            ),
            "high_24h": high_24h,
